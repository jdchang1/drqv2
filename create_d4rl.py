...
def get_policy_weights(agent):
    encoder_state_dict = agent.encoder.state_dict()
    actor_state_dict = agent.actor.state_dict()
    return dict(encoder=encoder_state_dict, actor=actor_state_dict)
...
def collect(env, agent, save_dir, num_episodes=200, num_ep_start=None):
    ...
    data_specs = (
        specs.Array((9, 64, 64), np.uint8, "raw"),
        env.action_spec(),
    ...
    while num_ep < num_episodes:
        ...
        with torch.no_grad(), utils.eval_mode(agent):
            action = agent.act(time_step.observation, 1600000, eval_mode=False)
...
def get_buffer(
    buffer_dir, save_dir, threshold, medium_dir, db_type="replay", num_episodes=200
):
    def get_time(path):
        fn = path.parts[-1]
...
def main(task, checkpoint, db_type, save_root, frame_size=64, n_eps=200):
    save_dir = save_root / db_type
    env = dmc.make(task, 3, 2, 0, frame_size=frame_size)
    agent = torch.load(checkpoint)["agent"]
    collect(env, agent, save_dir, num_episodes=n_eps)

    chkpt_save_dir = save_root / "checkpoints" / f"{db_type}.pt"
    chkpt = get_policy_weights(agent)
    with chkpt_save_dir.open("wb") as f:
        torch.save(chkpt, f)
...
if __name__ == "__main__":
    ...
    save_root = Path(f"../bcrl/data/{task}")
    ...
    main(task, random_agent, "random", save_root, frame_size=84)
